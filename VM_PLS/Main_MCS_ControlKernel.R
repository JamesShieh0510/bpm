Main_MCS_ControlKernel <- function(InTempRoute, InModelRoute){
  InSelectAlgorithm ="PLS"
  All_ConjectureID = 1
  TempConjectureTime =0
  
  load("../DataTransfer_TrainData.rda")
  ...
  for (NowGroup in 1:GroupSize){
    ...
    source('MCS_MR_Modeling.R')
    MCS_MR_Modeling(InX,InY,InTarget,NowGroup,InTempRoute,InModelRoute,InSelectAlgorithm)
    load("MCS_MR_Modeling.rda")
    
    TempContextID     = GroupInfo.list$ContextID
    TempPhaseID       = 2*matrix(1,LotNum,1)   # PhaseID, ID: 1:Stage1 & 2:Stage2
    TempPointID       = PointID
    ...
    #TempRT            = RT_Value
    TempConjectureTime = ConjectureTime + TempConjectureTime
  }
...
##---- ¡iRearrange Value¡j ----##
  ...
  save(OutAll_PredictValue, OutAll_ErrorValue, file = "MCS_ControlKernel.rda")
  
  }, warning = function(war) {
    print(paste("WARNING in Main_MCS_ControlKernel:  ",war))
    
  }, error = function(err) {
    print(paste("ERROR in Main_MCS_ControlKernel:  ",err))
